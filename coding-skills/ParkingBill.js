...
(where "HH" is a two-digit number between 0 and 23, which stands for hours,
...
because the entrance fee equals 2, the first hour costs 3 and there are two more full hours and part of a further hour,
so the total cost is 2 + 3 + (3 * 4) = 17. Given "09:42" and "11:42" your function should return 9,
because the entrance fee equals 2, the first hour costs 3 and the second hour costs 4,
...
// O(1)
function solution(E, L) {
  const [enterHour, enterMin] = E.split(':')
  const [leftHour, leftMin] = L.split(':')

  const mins = +leftMin - +enterMin
  let hours = +leftHour - +enterHour + (mins > 0 ? 1 : 0)
