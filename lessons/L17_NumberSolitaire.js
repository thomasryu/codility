...
that, given a non-empty array A of N integers, returns the maximal result that can
be achieved on the board represented by array A.
...
function solution(A) {
  ...
  // We build an array where each element is the maximum value
  // that can be achieved by reaching that element
  const max = Array(N).fill(-10000 * N)
  max[0] = A[0]

  // For each step, we get the current value in the maximum array,
  // add it to the value of the next 6 tiles (if they exist), and compare with their current values.
  // If the sum is higher then the current value, we replace it
  for (let i = 0; i < N; i++) {
    for (let j = 1; j <= 6 && i + j < N; j++) {
      max[i + j] = Math.max(max[i + j], max[i] + A[i + j])
    }
  }
