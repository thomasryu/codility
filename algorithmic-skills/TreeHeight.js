...
A binary tree is either an empty tree or a node (called the root) consisting of a
single integer value and two further binary trees, called the left subtree and the right subtree.
...
have the values 3 and 10, respectively. The right subtree of the node containing the value 10,
...
The attribute x holds the integer contained in the root, whereas attributes l and r hold
the left and right subtrees of the binary tree, respectively.
...
function solution(T) {
  ...
  const getHeight = (tree, height) => {
    ...
    getHeight(tree.l, height + 1)
    getHeight(tree.r, height + 1)
  }
