...
function solution(K, A) {
  ...
  // We just use a greedy algorithm to build the segments of size >= K since the first element
  // Proof: Imagine if the optimal solution did not include i = 0, either way, we could tie it to the
  // first segment either way and the solution would be the same. The same could be said about the X firt elements
  // so we can always include them in the solution
  A.forEach((a) => {
    if (sum + a >= K) {
      result++
      sum = 0
