...
function solution(M, A) {
  ...
  for (let end = 0; end < N; end++) {
    for (let start = end; start < N && !setA.has(A[start]); start++) {
      setA.add(A[start])
      result++
    }
    setA.clear()
...
function solution(M, A) {
  ...
  for (let end = 0; end < N; end++) {
    for (start; start < N && !setA.has(A[start]); start++) {
      setA.add(A[start])
    }
    result += setA.size
    setA.delete(A[end])
  ...
  return Math.min(MAX_RESULT, result)
}
